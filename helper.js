const verificar = (id) => {
    ...
    if (input.value.trim() == '') {
        ...
        if (id == 'fecha') {
            ...
            if (dia < 1) {
                input.classList.add('is-invalid')
                div.innerHTML = '<span class="badge bg-danger">La fecha de fundación de la banda no puede ser en el futuro</span>'
            }
        }

        /* validar que no se repita el id */
        if (id == 'codBanda') {
            if (!validarCod(input.value)) {
                input.classList.add('is-invalid')
                div.innerHTML = '<span class="badge bg-danger">Error</span>'
            }
        }
        /* if (id == 'run') {
...
const limpiar = () => {
    ...
    document.getElementById('codBanda').readOnly = false
...
console.log('hola')
const validarCod = (codBanda, codigos) => {
    console.log('hasldash')
    if (false) {
